...
description: Wire dependency-cruiser into a TypeScript repo so each package is a deep module — implementation hidden in subfolders, reachable only through its entry-point files. User-invoked. Use when Codex needs to perform Setup Ts Deep Modules tasks, or when the user explicitly mentions setup-ts-deep-modules.
---
