---
name: fastmcp
description: Build, test, inspect, install, and deploy MCP servers with FastMCP in Python. Use when creating a new MCP server, wrapping an API or database as MCP tools, exposing resources or prompts, or preparing a FastMCP server for an MCP-capable client or HTTP deployment.
---
...
- smoke-test a server with the FastMCP CLI before wiring it into the current agent or another client
- install a server into an MCP-capable client
- prepare a FastMCP server repo for HTTP deployment

When the server already exists and only needs to be connected, use the current agent's normal MCP configuration flow. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ${CODEX_HOME:-$HOME/.codex}/skills/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ${CODEX_HOME:-$HOME/.codex}/skills/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
- configure the server in the current agent's MCP configuration, or
...
The server-building part may be correct while the agent configuration is not. Configure the server in the current agent's MCP configuration, then restart the current agent if needed.
