---
name: computer-use
description: 'Drive the user''s desktop in the background — clicking, typing, scrolling, dragging — without stealing the cursor, keyboard focus, or switching virtual desktops / Spaces. Cross-platform: macOS, Windows, Linux. Works with any tool-capable model. Load this skill whenever the `computer_use` tool is available. Use when Codex needs to perform Computer Use tasks, or when the user explicitly mentions computer-use.'
---
