---
name: codexpp-public-equity-model-audit-tieout
...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is a polished standalone HTML model-audit report, while direct remediation preserves the source workbook format. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
For a substantive standalone HTML model-audit report, load `plugin-support/shared/html-artifact-standard.md` and use this audit-specific hierarchy:
...
- `codexpp-public-equity-memo-builder`: convert audit findings into a decision memo.
