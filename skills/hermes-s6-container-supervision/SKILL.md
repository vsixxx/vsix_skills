...
description: Modify, debug, or extend the s6-overlay supervision tree inside the Hermes Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern. Use when Codex needs to perform Hermes S6 Container Supervision tasks, or when the user explicitly mentions hermes-s6-container-supervision.
---
