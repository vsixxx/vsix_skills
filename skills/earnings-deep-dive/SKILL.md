...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is a polished standalone HTML post-earnings report. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
Default to the full post-print package. A new standalone reader-facing post-print output should be a polished standalone HTML post-earnings report following `plugin-support/shared/html-artifact-standard.md`; use chat only when the user explicitly requests a lightweight response. Use `dashboard-builder` only for the optional standardized-dashboard route below. Use deterministic file mode only when the user supplies `plan.json`, normalized CSVs, model-update inputs, or explicitly asks for files.
...
For a substantive HTML deep dive, load `plugin-support/shared/html-artifact-standard.md` and let the company-specific investment debate determine the layout.
