---
name: codexpp-public-equity-three-statement-model-builder
...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is an XLSX three-statement model workbook. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
Use this skill when the deliverable is a 3-statement build, refresh, rerun, operating model package, or integrated forecast rebuild. Route away when the primary ask is workbook audit/debug (`codexpp-public-equity-model-audit-tieout`), source hierarchy (`financial-source-of-truth`), raw financial normalization (`codexpp-public-equity-financials-normalizer`), spreadsheet cleanup (`excel-data-cleaner`), valuation (`codexpp-public-equity-dcf-model-builder`, `codexpp-public-equity-comps-valuation`), public estimate update (`equity-model-update`), Credit Markets research, or memo/deck polish.
...
Load `shared/support-layer-routing-contract.md` when support services are needed. Use `financial-source-of-truth` for filings, releases, guidance, consensus, market data, and assumption labels; use `codexpp-public-equity-financials-normalizer` before building the plan from messy financials, KPI schedules, segment tables, share count, net debt, capital allocation, or provider exports; use `excel-data-cleaner` before relying on malformed workbook tables; use `codexpp-public-equity-model-audit-tieout` for workbook logic, external links, recalc/cache posture, and final tie-out. Support artifacts stay secondary to the formula workbook or owning dashboard/report.
