...
description: Ingest a voice note with exact-phrasing preservation (never paraphrased). Routes content to originals/, concepts/, people/, companies/, ideas/, personal/, or voice-notes/ based on a decision tree. The user's exact words are the signal. Use when Codex needs to perform Voice Note Ingest tasks, or when the user explicitly mentions voice-note-ingest.
---
