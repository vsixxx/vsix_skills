...
**问题：** LLM 经常隐藏动作的执行者，或者完全省略主语，例如“无需配置文件”“结果会自动保留”。主动表达更清楚时，应明确是谁做了什么。
...
**问题：** 英文 AI 文本常把常见词组统一写成带连字符的复合修饰语，例如 `data-driven`、`client-facing` 和 `end-to-end`。处理英文时，不要为了形式整齐机械地添加连字符；技术术语和确有歧义的复合修饰语除外。中文通常不受此模式影响。

---

### 27. 故作权威的引导语

**需要注意的表达：** 真正的问题是、归根结底、从根本上说、真正重要的是、更深层的问题、问题的核心

**问题：** 这些表达假装要揭示更深刻的结论，但后面的内容往往只是普通观点的重复。

**改写前：**
> 真正的问题是团队能否适应。归根结底，真正重要的是组织是否准备好了。

**改写后：**
> 问题在于团队能否适应，这主要取决于组织是否愿意改变现有习惯。

---

### 28. 预告和报幕式表达

**需要注意的表达：** 让我们深入探讨、让我们来看看、下面进行拆解、以下是你需要知道的、话不多说

**问题：** 与其直接提供内容，LLM 经常先宣布接下来要做什么。这类元叙述会拖慢节奏，让文章像教程脚本。

**改写前：**
> 让我们深入了解 Next.js 的缓存机制。以下是你需要知道的内容。
...
**需要注意的结构：** 标题后紧跟一句只是在复述标题的短句，然后才进入真正内容。

**问题：** 这种热身句没有增加信息，只会让文章显得填充感很强。
...
4. 确保修订后的文本：
   ...
   - 为上下文保持适当的语气
   - 适当时使用简单的结构（是/有）
5. 检查是否为了增加“人味”而编造了经历、引语、研究或具体事实；如有，删除或改回可验证的表述
6. 问自己：“这段文字还有哪些地方明显像 AI 写的？”
7. 根据检查结果完成最后一轮修订，再呈现最终版本

## 输出格式

默认只提供最终重写文本。诊断过程、评分或修改摘要仅在用户要求或确实有助于审阅时提供，不要让解释淹没正文。
...
本中文指南由 [op7418/Humanizer-zh](https://github.com/op7418/Humanizer-zh) 本地化内容与 [blader/humanizer](https://github.com/blader/humanizer) 2.5.1 版合并而成，并补齐较新版本中的 29 类模式。原始指南基于 [Wikipedia: Signs of AI writing](https://en.wikipedia.org/wiki/Wikipedia:Signs_of_AI_writing)，由 WikiProject AI Cleanup 维护。
