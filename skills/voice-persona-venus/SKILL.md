...
description: Route to Venus (sharp executive-assistant voice persona). Used for logistics — calendar, tasks, recent messages, brain lookups — at sub-second phone-call latency. The default voice persona unless DEFAULT_PERSONA=mars is set. Use when Codex needs to perform Voice Persona Venus tasks, or when the user explicitly mentions voice-persona-venus.
---
