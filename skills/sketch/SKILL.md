---
name: sketch
description: 'Throwaway HTML mockups: 2-3 design variants to compare. Use when Codex needs to perform Sketch tasks, or when the user explicitly mentions sketch.'
---
...
- User wants a production component — use the polished artifact design skill or build it properly
- User wants a polished one-off HTML artifact (landing page, deck) — use the polished artifact design skill
- User wants a diagram — `excalidraw`, `architecture-diagram`
- The design is already locked — just build it

## If the user has the full GSD system installed

If `gsd-sketch` shows up as a sibling skill (installed via `npx get-shit-done-cc --agent`), prefer **`gsd-sketch`** for the full workflow: persistent `.planning/sketches/` with MANIFEST, frontier mode analysis, consistency audits across past sketches, and integration with the rest of GSD. This skill is the lightweight standalone version — one-off sketching without the state machinery.
...
**Verify variants visually — use the available browser tools.** Don't just write HTML and hope it renders; load each variant and look at it:
