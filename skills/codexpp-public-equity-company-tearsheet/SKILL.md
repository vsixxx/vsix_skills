---
name: codexpp-public-equity-company-tearsheet
...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is a polished standalone HTML issuer tearsheet. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
Use this before or inside comps, DCF, 3-statement, earnings, model update, long/short pitch, memo, codexpp-public-equity-meeting-prep, thesis tracker, risk/sizing, hedge, event, catalyst, equity-risk credit-signal, and deck/report workflows when a fast public profile is needed.

Produce a compact polished standalone HTML tearsheet following `plugin-support/shared/html-artifact-standard.md` unless another format takes precedence; let the issuer's business model and requested review lens determine the hierarchy. Use chat only when the user explicitly requests a lightweight response. If the work expands into detailed thesis construction, scenarios, underwriting, or a full investment view, route the baseline into `initiating-coverage`, `codexpp-public-equity-memo-builder`, or the relevant owning workflow. Use `dashboard-builder` only when the user explicitly asks for a standardized dashboard, reusable dashboard template, or structured payload-driven render.
...
- Use `codexpp-public-equity-financials-normalizer` or `excel-data-cleaner` first when source financials/tables are messy.
...
For standardized dashboard handoffs only, use `references/DASHBOARD_PACK.md`. `codexpp-public-equity-company-tearsheet` owns the issuer baseline and source confidence; `dashboard-builder` owns the shared shell/rendering/QA. Build a `public_equity_investing_dashboard.v1` payload as an internal renderer input, and keep JSON/Markdown support files behind the HTML artifact unless explicitly requested.

## When To Invoke Support

Load `shared/support-layer-routing-contract.md` when support services are needed. Use `financial-source-of-truth` for source hierarchy, stale data, conflicts, and fact/assumption labels; use `codexpp-public-equity-financials-normalizer` or `excel-data-cleaner` before using messy financials, ownership tables, market/security data, or KPI exports. Support artifacts stay secondary to the tearsheet, dashboard/report, or downstream owning workflow.
...
When HTML is requested or selected, load `plugin-support/shared/html-artifact-standard.md` and apply these workflow-specific rules:
...
- Do not turn a tearsheet into a recommendation; route investment decisions to initiating coverage, codexpp-public-equity-memo-builder, long-short-pitch, or thesis-tracker.
