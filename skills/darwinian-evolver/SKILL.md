...
description: Evolve prompts/regex/SQL/code with Imbue's evolution loop. Use when Codex needs to perform Darwinian Evolver tasks, or when the user explicitly mentions darwinian-evolver.
---
...
import upstream classes into the current agent itself.
...
```bash
mkdir -p ~/.codex/vsix-skill-cache/darwinian-evolver && cd ~/.codex/vsix-skill-cache/darwinian-evolver
...
```bash
cd ~/.codex/vsix-skill-cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.codex/vsix-skill-cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=${CODEX_HOME:-$HOME/.codex}/skills/darwinian-evolver
DE_DIR=~/.codex/vsix-skill-cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside the current agent core.
   Custom driver scripts under `$CODEX_HOME/skills/...` are user-side and fine.
