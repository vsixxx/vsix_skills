...
description: Free web search via DuckDuckGo — text, news, images, videos. No API key needed. Prefer the `ddgs` CLI when installed; use the Python DDGS library only after verifying that `ddgs` is available in the current runtime. Use when Codex needs to perform Duckduckgo Search tasks, or when the user explicitly mentions duckduckgo-search.
---
