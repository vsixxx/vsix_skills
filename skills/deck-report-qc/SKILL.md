...
For a substantial standalone HTML QC report, load `plugin-support/shared/html-artifact-standard.md`. Let the evidence, circulation question, and highest-impact findings determine the hierarchy rather than forcing the review into a fixed dashboard module inventory.

## Embedded Support Routing

This is an embedded service under the owning workflow unless the user explicitly asks for standalone deck/report QC. Preserve the `owning_workflow` internally, such as `codexpp-public-equity-memo-builder`, `long-short-pitch`, `initiating-coverage`, `earnings-preview`, `earnings-deep-dive`, `economic-impact-report`, `equity-model-update`, `codexpp-public-equity-dcf-model-builder`, `codexpp-public-equity-three-statement-model-builder`, `codexpp-public-equity-comps-valuation`, `thesis-tracker`, `codexpp-public-equity-meeting-prep`, or `dashboard-builder`.
...
6. Route model-level issues to `codexpp-public-equity-model-audit-tieout` and data-shaping issues to `excel-data-cleaner` instead of trying to solve them inside this skill.
...
Default output should be a senior-review QC readout. For standalone QC-only work, produce a polished standalone HTML senior-review QC report following `plugin-support/shared/html-artifact-standard.md`. Use chat only when the user explicitly requests a lightweight response. When the script is used, the reader-facing artifact should be `public_equity_investing_deck_qc_report.html`; CSV, JSON, manifests, payloads, and support notes are audit/import support unless the user asks for them.
...
- workbook formula, model logic, sensitivity, scenario, source tie-out -> `codexpp-public-equity-model-audit-tieout`
- messy tabular data, duplicated rows, bad date/number formats -> `excel-data-cleaner`
- valuation model construction or repair -> `codexpp-public-equity-dcf-model-builder`, `codexpp-public-equity-comps-valuation`, or `codexpp-public-equity-three-statement-model-builder`
...
- final IC synthesis -> `codexpp-public-equity-memo-builder`
