...
description: Control a running TouchDesigner instance via twozero MCP — create operators, set parameters, wire connections, execute Python, build real-time visuals. 36 native tools. Use when Codex needs to perform Touchdesigner MCP tasks, or when the user explicitly mentions touchdesigner-mcp.
---
...
```
the current agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${CODEX_HOME:-$HOME/.codex}/skills/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to agent configuration (if missing)
...
3. **Restart the current agent session** to pick up the new MCP server
