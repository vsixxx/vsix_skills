...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is a polished standalone HTML idea-triage report. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
For a new standalone reader-facing idea screen, market map, watchlist review, or candidate set, produce a polished standalone HTML idea-triage report following `plugin-support/shared/html-artifact-standard.md`; let the mandate and screening question determine the structure. Use chat only when the user explicitly requests a lightweight response. Use `dashboard-builder` only when the user explicitly asks for a standardized dashboard, reusable dashboard template, or structured payload-driven render.
...
For a substantive HTML idea-triage report, load `plugin-support/shared/html-artifact-standard.md` and apply these workflow-specific rules:
