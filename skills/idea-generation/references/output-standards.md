...
Use this reference when producing a PM pack, idea cards, exportable idea log, or examples of senior-investor language. For a substantial screen, market map, watchlist review, or source-heavy candidate set, default to a flexible standalone HTML idea-triage report following `../../plugin-support/shared/html-artifact-standard.md`; use the standardized dashboard pack only when that dashboard path is explicitly selected.
