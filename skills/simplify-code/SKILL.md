---
name: simplify-code
description: Parallel 3-agent cleanup of recent code changes. Use when Codex needs to perform Simplify Code tasks, or when the user explicitly mentions simplify-code.
---
...
- "/simplify" (if they're carrying that command habit over)
...
- **Respect project conventions.** If the repo has AGENTS.md / CLAUDE.md /
  other project instruction files or a linter config, fold those rules into the reviewer prompts so
