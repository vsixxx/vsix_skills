...
If `surface = chatgpt_web` and `mode = work_mode` are both positively identified, do not render with Data Analytics MCP chart or table widgets and do not select an MCP artifact as the visual surface. Use native Work Mode chart or table rendering when available for inline results. For durable HTML reports and dashboards, use the static Seaborn-to-PNG path required by those surfaces. MCP servers and other callable tools remain valid data sources.
...
4. Select the delivery path that matches the final surface.
   - In positively identified web Work Mode, use native Work Mode chart or table rendering when available for ad hoc, inline, chat-visible, or otherwise unspecified visuals; if native rendering is unavailable, use a compact table or a static chart that fits the selected non-MCP surface.
...
- Do not ship underpowered trend charts. For line, area, sparkline, codexpp-data-analytics-indexed-trend, or other time-series views, first try to expose either a finer time grain or a longer date range so the displayed series has enough observed points to show shape. As a default, aim for at least 8-12 temporal points for a trend visual; if the available evidence has only a few anchor periods, query daily or weekly rows, extend the lookback, or add a meaningful segment/comparator before rendering. Use a KPI strip, grouped bar, slope chart, table, or narrative callout instead when the question is really a small set of discrete period comparisons.
...
- Do not use leaderboards to rank KPI definitions or time-window definitions against each other, such as latest DAU versus WAU versus MAU. Use KPI cards or a compact table for latest values; use trends, codexpp-data-analytics-indexed trends, share trends, or ratio charts for movement or relationship questions.
...
- For active-user dashboards, use KPI cards for latest DAU, WAU, MAU, Plus DAU, and Plus WAU values; trend or codexpp-data-analytics-indexed-trend charts for movement over time; share or stickiness views when the question is mix or ratios; and categorical horizontal bars for unrelated week-over-week metric movements.
...
- For data visualization requests outside web Work Mode where the user has not chosen a file, notebook, BI/dashboard, report, slide, docs, HTML, or no-render output surface, render with Data Analytics MCP chart widgets by default when the MCP widget tools are available; in web Work Mode use native chart or table rendering when available, otherwise use the selected non-MCP fallback. When the user explicitly chooses another surface, keep this skill's output to chart selection, data planning, implementation guidance, and QA for that surface.
- When the selected delivery surface is an MCP Apps widget or Data Analytics MCP report/dashboard artifact, read `plugin-support/src/analytics-app-core.md` after selecting the chart family. Do not let an MCP-supported type list drive general chart selection for non-MCP surfaces.
