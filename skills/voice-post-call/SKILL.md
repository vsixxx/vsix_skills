...
description: 'Post-call handling for a voice session — turn the transcript into a brain page, post the summary to the operator''s messaging surface, archive the audio. Belt-and-suspenders: fires both from a tool the voice persona can call mid-call AND from the automatic call-end handler in server.mjs. Use when Codex needs to perform Voice Post Call tasks, or when the user explicitly mentions voice-post-call.'
---
