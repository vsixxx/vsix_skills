...
description: 'Review PRs: diffs, inline comments via gh or REST. Use when Codex needs to perform GitHub Code Review tasks, or when the user explicitly mentions github-code-review.'
---
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _agent_env="a local `.env` file or shell environment"; [ -f "$_agent_env" ] && grep -q "^GITHUB_TOKEN=" "$_agent_env"; then
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from the current agent\",
    \"comments\": [
...
```bash
source "${CODEX_HOME:-$HOME/.codex}/skills/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by the current agent. Code looks clean — good test coverage, no security concerns."
