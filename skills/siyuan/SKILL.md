---
name: siyuan
description: SiYuan Note API for searching, reading, creating, and managing blocks and documents in a self-hosted knowledge base via curl. Use when Codex needs to perform Siyuan tasks, or when the user explicitly mentions siyuan.
---
...
3. Store it in `a local `.env` file or shell environment`:
   ```
...
```yaml
# In the current agent's MCP configuration:
mcp_servers:
  siyuan:
    command: npx
