...
4. CRM/deal tracker/project tracker status.
5. Prior meeting notes or transcripts.
6. Inference from context, labeled as an assumption.

### Company, deal, or investment facts
1. User-provided source documents and active artifacts.
2. Primary source materials: company financials, management presentations, data room files, contracts, board materials, filings, press releases, earnings releases, transcripts, investor presentations.
3. Connected financial or market data apps available to the user.
4. Internal research, prior memos, prior models, CRM notes, committee materials, and diligence trackers.
...
2. Recent emails, chat messages, prior meeting transcripts, CRM notes, and relationship notes.
...
## 2. Connected-source intake
When connectors are available, gather only what materially improves the prep. Useful connector categories include:
...
- drive/docs/slides/sheets: pre-reads, models, decks, memos, diligence trackers, templates;
- transcripts/meeting records: prior questions, commitments, unresolved issues, tone;
- CRM/deal/process trackers: stage, owners, next milestones, buyer/lender/investor status;
- finance/data apps: filings, earnings, consensus, market data, portfolio reports, ERP/FP&A data, lender materials, cap table, data room files.

Do not ask for files or dates that are clearly available in the connected context. If connectors are unavailable or insufficient, say what was missing and proceed with a fallback brief.

## 3. Stale-data checks
Flag data as potentially stale when:
- meeting logistics differ across invite, email, and user prompt;
- financials predate a newer filing, earnings release, budget, forecast, lender package, or data room upload;
...
- deal status, process deadlines, buyer list, diligence status, or action trackers are older than the latest process communication;
...
## 4. Citation and source-log behavior
Cite facts that anchor the prep: financials, transaction terms, dates, deadlines, guidance, process status, meeting objective, prior commitments, and diligence responses. Generic questions and judgment do not need citations, but should be traceable to the relevant issue.
...
Example phrasing:
- "Revenue is shown as $120m in the latest management deck but $116m in the older model. I used $120m for the brief and added a pre-call tie-out item."
- "The calendar invite says this is a lender update, while the email thread frames it as a covenant waiver discussion. Treating waiver terms as the likely decision point; verify at the start."

## 6. Fact, assumption, and inference labels
Use these labels consistently:
- **Verified fact**: supported by a cited source.
- **Management claim**: stated by company/borrower/issuer/management but not independently verified.
