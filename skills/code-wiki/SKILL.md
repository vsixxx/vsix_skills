---
name: code-wiki
description: Generate wiki docs + Mermaid diagrams for any codebase. Use when Codex needs to perform Code Wiki tasks, or when the user explicitly mentions code-wiki.
---
...
Invoke through the `terminal` tool from the target repo's root, then use `read_file` / `search_files` / `write_file` to produce the wiki. Default output location is `~/.codex/vsix-skill-output/wikis/<repo-name>/`. Only write into the repo (`docs/wiki/`) when the user explicitly requests it.
...
```bash
OUTPUT_DIR="$HOME/.agent/wikis/$REPO_NAME"
...
1. **User input** — [`cli.py:AgentCLI.run_session`](<link>)
...
cat > "$OUTPUT_DIR/.codewiki-state.json" <<EOF
{
  ...
  "generator": "agent code-wiki skill v0.1.0",
  "modules_documented": []
}
EOF
```
...
```
Generated wiki at ~/.codex/vsix-skill-output/wikis/<repo-name>/:
...
- **In-repo output without asking.** Default is `~/.codex/vsix-skill-output/wikis/`. Only write into the repo when the user explicitly requests it.
