---
name: codexpp-public-equity-financials-normalizer
...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is an XLSX normalization workbook. Do not choose chat-only output unless the user explicitly requests a lightweight response.

When invoked as support for an owning workflow, inherit its resolved deliverable preferences and do not re-prompt. Only when this skill independently owns a new standalone reader-facing normalization deliverable should it, before source gathering, analysis, or rendering, load `plugin-support/shared/deliverable-intake-policy.md` and perform its adaptive `request_user_input` preflight for materially unresolved preferences.
...
This is an embedded service under the owning workflow unless the user explicitly asks for standalone normalization. Preserve the `owning_workflow` internally, such as `equity-model-update`, `codexpp-public-equity-dcf-model-builder`, `codexpp-public-equity-three-statement-model-builder`, `codexpp-public-equity-comps-valuation`, `earnings-preview`, `earnings-deep-dive`, `codexpp-public-equity-memo-builder`, `thesis-tracker`, `codexpp-public-equity-scenario-sensitivity-generator`, `portfolio-risk-management`, or `dashboard-builder`.
