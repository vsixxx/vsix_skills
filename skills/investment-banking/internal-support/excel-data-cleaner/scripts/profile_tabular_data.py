...
OPENPYXL_REVIEW_FALLBACK = (
    ...
    "then preserve raw data, inferred headers/types, quality issues, and assumptions while profiling manually."
)
...
def ensure_openpyxl_for_excel_input(path: Path) -> None:
    ...
    try:
        ...
        if exc.name == "openpyxl":
            raise DependencyPreflightError(
                openpyxl_dependency_message(f"read {path.suffix.lower()} workbook inputs")
            ) from exc
        raise
...
def infer_domain(headers: list[str], sheet_name: str, data_values: Iterable[str]) -> dict[str, Any]:
    ...
    return {"best_guess": best, "scores": scores, "matched_keywords": matched}
...
def main() -> None:
    ...
    try:
        ensure_openpyxl_for_excel_input(path)
