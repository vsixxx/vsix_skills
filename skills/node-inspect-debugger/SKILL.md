...
description: Debug Node.js via --inspect + Chrome DevTools Protocol CLI. Use when Codex needs to perform Node Inspect Debugger tasks, or when the user explicitly mentions node-inspect-debugger.
---
...
Agent-runtime note: `chrome-remote-interface` is NOT in `ui-tui/package.json`. Install it to a throwaway location if you don't want to dirty the project:
...
## Debugging the current agent ui-tui
...
6. **Running `node inspect` through an agent terminal.** It's a PTY-friendly REPL. In the current agent, launch it with `terminal(pty=true)` or `background=true` + `process(action='submit', data='...')`. Non-PTY foreground mode will work for one-shot commands but not for interactive stepping.
