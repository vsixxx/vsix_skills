...
Save reusable Public Equity Investing context in `$CODEX_HOME/state/plugins/oai-maintained-plugins/public-equity-investing/user-context.md`. Prefer the best matching existing scaffold category. Add a concise new category only when no existing category fits cleanly.
