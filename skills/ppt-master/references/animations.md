...
python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project> -a auto

# Enable with a single effect (cascades via the after-previous trigger)
python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project> --animation fade

# Enable and switch to on-click for live presentations (presenter controls pacing)
python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project> -a auto --animation-trigger on-click

# Custom pacing
python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project> --animation mixed \
        --animation-stagger 0.7 --animation-duration 0.5

# All groups animate in unison on slide entry
python3 ${SKILL_DIR}/scripts/svg_to_pptx.py <project> -a auto --animation-trigger with-previous
```
