...
| **Needs-Manual** | Legacy compatibility state for an explicitly manual project | Dashed placeholder unless the user supplied the file. Normal agent-native generation keeps a missing required row `Pending` and stops before Executor. |
...
2. Image Acquisition (Step 5):
   - Pending + ai  → current agent image tool / installed image Skill → Generated
