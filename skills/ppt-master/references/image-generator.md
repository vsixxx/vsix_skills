...
Write `project/images/image_prompts.json` per §6, then follow §7. The current agent invokes its available native image tool or installed image-generation Skill directly.
...
For traceability, add optional `slice_grid` and `slice_names` fields to the sheet item in `image_prompts.json` after choosing the geometry. These fields document the exact command that must be used for slicing.
...
1. **Flat background, matched to the slide.** Unless the available image tool explicitly supports transparent output, the cut element carries whatever was behind it. A flat sheet background (= deck background HEX) is what `--alpha` keys out and what makes non-keyed pieces blend.
...
| `items[].aspect_ratio` | yes | Container sizing | Passed to the available image-generation tool when supported |
...
> Prerequisite: §3 Steps 1-3 complete and `images/image_prompts.json` exists. The manifest remains the audit and handoff contract even though this Skill does not call image-provider APIs directly.

### Capability Selection

1. Detect whether the current agent exposes an image-generation tool or has an installed image-generation Skill. Agent-native image tools and Skills such as `vsix-image-gen` both satisfy this requirement.
2. If a capability exists, invoke it directly for each `Pending` item using `items[].prompt`, reference images, aspect ratio, and requested dimensions when supported.
3. If no capability exists, stop the image-generation stage and tell the user that the current agent has no available image-generation capability. List the expected filenames and keep the prepared prompts in `images/image_prompts.json`.
4. Do not request or configure Gemini, OpenAI, Qwen, Zhipu, Volcengine, or other image-provider API keys from this Skill.
5. Do not silently switch to web image search. Web sourcing is a separate `Acquire Via: web` decision and runs only when the user or confirmed design plan selected it.

### Execution Rules

- Generate in modest groups only when the available tool supports independent parallel calls; otherwise process serially.
- Outputs must land at `project/images/<filename-from-resource-list>`.
- Use the closest supported native size and record actual pixel dimensions. Do not upscale merely to imitate the requested size.
- Mark an item `Generated` only after its output file exists.
- Retry a failed generation once with the same available capability. If it still fails, stop and report the filename, prompt, and error; do not switch providers or acquisition modes automatically.
- Downstream execution is capability-agnostic and consumes only the resulting files and manifest state.

### Unavailable Capability

When no image-generation capability is available:

1. Keep every affected item as `Pending`.
2. Preserve `images/image_prompts.json` and provide the missing filenames to the user.
3. Stop before Executor if those images are required by the confirmed design.
4. Resume this section after the user enables an image-generation Skill/tool or changes the confirmed image plan.

**Hard rule**: do not claim an image was generated without an actual file at the expected path, and do not turn an unavailable image tool into an implicit web-search decision.
...
2. Re-run the current agent's available image-generation capability for that item only
