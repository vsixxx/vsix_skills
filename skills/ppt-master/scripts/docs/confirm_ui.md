...
- **Closed enumerable** — PPT reading mode (`delivery_purpose` compatibility key), formula policy / generation mode / refine spec. These have no Custom box; out-of-catalog values snap back to the recommended option.
...
- When confirmed Stage-2 `image_usage` includes `ai`, the current agent later uses its available native image tool or installed image-generation Skill. The confirmation page does not ask users to choose an API backend.
...
- `image_strategy` appears only with `image_usage: ai` and remains confirmed downstream. The available Agent capability executes the confirmed strategy; [`strategist-image.md`](../../references/strategist-image.md) §2 locks the chosen strategy verbatim.
