...
python3 -c "from playwright.sync_api import sync_playwright; p=sync_playwright().start(); b=p.chromium.launch(headless=True); b.close(); p.stop()" \
  || { echo "Playwright and Chromium are required for visual review" >&2; exit 1; }
...
**Agent compatibility**: when the current agent supports subagents, it may dispatch independent review batches with minimal task-local context. Otherwise the main agent processes the same batches sequentially. The review contract and evidence requirements do not change.
