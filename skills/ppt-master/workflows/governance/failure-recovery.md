...
| AI image generation failure | Yes when the confirmed design requires the image | Retry the current agent's available image capability once; never switch to an API backend or web search automatically | Enable an image-generation Skill/tool or change the confirmed image plan | Step 5 image generation |
