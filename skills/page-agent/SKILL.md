---
name: page-agent
description: Embed alibaba/page-agent into your own web application — a pure-JavaScript in-page GUI agent that ships as a single script tag or npm package and lets end-users of your site drive the UI with natural language. Use this skill when the user is a web developer who wants to add an AI copilot to a web app, make a legacy app accessible via natural language, or evaluate page-agent against a local or cloud LLM. Not for server-side browser automation. Use when Codex needs to perform Page Agent tasks, or when the user explicitly mentions page-agent.
---
...
- User wants **the current agent itself to drive a browser** → use the current agent's built-in browser tool. page-agent is the *opposite* direction.
