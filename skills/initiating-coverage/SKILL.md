...
Apply the presentation-surface precedence in `plugin-support/shared/deliverable-intake-policy.md`. This workflow's natural artifact is a polished standalone HTML initiation report. Do not choose chat-only output unless the user explicitly requests a lightweight response.
...
- IC memo, investment memo, committee note, client memo, PM update, or memo rewrite: use `codexpp-public-equity-memo-builder`.
...
- One-page public issuer profile: use `codexpp-public-equity-company-tearsheet`.
...
4. **Coordinate with local skills.** Use `financial-source-of-truth` for evidence posture, `codexpp-public-equity-company-tearsheet` for issuer baseline, `codexpp-public-equity-financials-normalizer` or `excel-data-cleaner` for source prep, model skills for valuation work, `codexpp-public-equity-scenario-sensitivity-generator` for cases, and `thesis-tracker` after initiation.
