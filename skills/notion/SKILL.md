---
name: notion
description: 'Notion API + ntn CLI: pages, databases, markdown, Workers. Use when Codex needs to perform Notion tasks, or when the user explicitly mentions notion.'
---
...
3. Store in `a local `.env` file or shell environment`:
   ```
...
Add those exports to your shell profile (or to `a local `.env` file or shell environment`) so every session inherits them.
...
curl -s -X PATCH "https://api.notion.com/v1/blocks/{page_id}/children" \
  ...
  -d '{
    "children": [
      {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": "Hello from the current agent!"}}]}}
    ]
  }'
```
...
- Notion also ships an MCP server now (`Notion MCP`, ~91% more token-efficient on DB ops than the previous version) — wire it through the current agent's MCP support if you want streaming Notion access from inside a session, but the paths above are enough for most one-shot tasks.
