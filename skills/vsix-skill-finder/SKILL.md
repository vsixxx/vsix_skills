...
7. Branch on `distribution.kind`:
   ...
   - `plugin`: inspect configured Marketplaces first. Add the declared Marketplace only when its name is absent. Reuse or upgrade it when the name already points to the declared URL. For `marketplaceManaged: true`, inspect `marketplaceName` and `marketplaceAliases`, then use whichever Codex-managed official Marketplace exists and contains the plugin. These reserved Marketplaces must not be added, replaced, or overwritten manually; if none exists, stop and ask the user to update Codex, verify their login mode, and restart or begin a new conversation. Inspect installed plugins, then install or update the qualified ID for the selected Marketplace only when needed; an installed current version is success.
